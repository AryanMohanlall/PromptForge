...
public abstract class ABPGroupWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
{
    protected static readonly Lazy<string> ContentRootFolder;

    static ABPGroupWebTestBase()
    {
    ...
    protected void UsingDbContext(Action<ABPGroupDbContext> action)
    {
        using (var context = IocManager.Resolve<ABPGroupDbContext>())
        {
            action(context);
            context.SaveChanges();
        }
    ...
    protected T UsingDbContext<T>(Func<ABPGroupDbContext, T> func)
    {
        T result;
    ...
    protected async Task UsingDbContextAsync(Func<ABPGroupDbContext, Task> action)
    {
        using (var context = IocManager.Resolve<ABPGroupDbContext>())
        {
    ...
    protected async Task<T> UsingDbContextAsync<T>(Func<ABPGroupDbContext, Task<T>> func)
    {
        T result;
